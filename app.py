...
if st.session_state.get("authentication_status"):
    ...
    with col_controles:
        ...
        # OPCIÓN RESTAURADA: Mostrar/Ocultar nombres
        mostrar_nombres = st.toggle("🏷️ Ver Nombres", value=True)

        st.markdown("---")
        archivo = st.file_uploader("Sube tu Excel", type=["xlsx"])
        
        if archivo:
            # Forzar carga y centrado si el archivo cambia o no hay datos
            if st.session_state.get('last_file') != archivo.name:
                df_raw = pd.read_excel(archivo)
                df_raw.columns = df_raw.columns.str.strip()
                
                # Normalizar columnas
                renom = {'lat':'Latitud','latitud':'Latitud','lon':'Longitud','longitud':'Longitud','lng':'Longitud'}
                df_proc = df_raw.rename(columns=renom).dropna(subset=['Latitud', 'Longitud'])
                
                if 'Volumen' not in df_proc.columns: df_proc['Volumen'] = 0
                
                # REGLA DE ORO: Centrar mapa en el promedio de los datos
            ...
            for _, fila in df_ver.iterrows():
                ...
                if mostrar_nombres:
                    folium.Marker(
                        [fila['Latitud'], fila['Longitud']], 
                        icon=DivIcon(html=f"""
                            <div style="font-size: 8pt; font-weight: bold; color: black !important; 
                            text-shadow: 1px 1px 2px white; width: 100px; text-align: center;">
                                {fila.get('Nombre','')}
                            </div>""")
                    ).add_to(m)

            st_folium(m, width="100%", height=700, key="mapa_final_v2")
        else:
            st.info("Sube un archivo para que el mapa se centre automáticamente en tus puntos.")
